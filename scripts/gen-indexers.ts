...
// indexer with more than MIN_STAKE_GRT staked (default 100000). This is the prune: indexers below the
// floor (incl. deregistered/unstaked ones) are dropped. Name preference: ENS primary name (reverse
// resolution on mainnet) → network-subgraph defaultDisplayName → URL host. Run OUT-OF-BAND (cron/CI);
// the exporter only reads the file. Usage:
...
const MIN_STAKE_GRT = Number(process.env.MIN_STAKE_GRT ?? 100000);
...
// Cursor-paginate registered indexers over the stake floor. id_gt keeps paging stable on large sets.
